...
const ChannelModal = ({ visible, onClose }) => {
  const { myChannels, moreChannels, onUpdateChannel } = useContext(ChannelContext)
  ...
  return (
    <div className="channels" style={{ 'display': visible ? 'block' : 'none' }}>
      ...
      <div className="content">
        ...
        <ChannelHeader title={'更多版块'} info={'点击添加版块'} />
        <div className="list">
          {moreChannels.length > 0 ? moreChannels.map(item =>
            <ChannelItem
              key={item.id}
              name={item.name}
              className="more-item"
            ...
            <div className="no-more">已全部添加至我的版块</div>
          )}
        </div>
      </div>
